...
_BASIC_TYPES = {str, int, float, bool, type(None)}
...
def convert_and_respect_annotation_metadata(
    *,
    ...
    # Pydantic models
    origin_type = typing_extensions.get_origin(clean_type)

    # Fast path: basic types and no struct annotation
    if type(object_) in _BASIC_TYPES and origin_type is None:
        return object_
    ...
    # Container types lookups
    dict_types = {typing.Dict, dict}
    set_types = {typing.Set, set}
    list_types = {typing.List, list}
    sequence_types = {typing.Sequence, collections.abc.Sequence}

    if (origin_type in dict_types or clean_type in dict_types) and isinstance(object_, typing.Dict):
    ...
    if not isinstance(object_, str):
        if (origin_type in set_types or clean_type in set_types) and isinstance(object_, typing.Set):
            args = typing_extensions.get_args(clean_type)
            inner = args[0]
            return {
            ...
            (origin_type in list_types or clean_type in list_types) and isinstance(object_, typing.List)
        ) or (
            (origin_type in sequence_types or clean_type in sequence_types) and isinstance(object_, typing.Sequence)
        ):
            args = typing_extensions.get_args(clean_type)
            inner = args[0]
            return [
    ...
    if origin_type == typing.Union:
        ...
        for member in typing_extensions.get_args(clean_type):
            # Only auto-recurse if not a basic type
            if _remove_annotations(member) not in _BASIC_TYPES:
